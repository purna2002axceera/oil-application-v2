...
import { Table, Button, InputNumber, Select, Input } from 'antd'
...
const ReturnPage = () => {
  ...
  useEffect(() => {
    applyFilters()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchReturnNo, searchCreatedAt, allReturns])
  ...
  const applyFilters = () => {
    ...
    if (searchReturnNo) {
      filtered = filtered.filter(ret => 
        ret.ronumber?.toLowerCase().includes(searchReturnNo.toLowerCase())
      )
    }

    if (searchCreatedAt) {
      filtered = filtered.filter(ret => 
        ret.createdAt?.split('T')[0] === searchCreatedAt
      )
    }
  ...
  const mainColumns = [
    { title: 'Return No', dataIndex: 'ronumber', key: 'ronumber', width: '40%' },
    { title: 'Created At', dataIndex: 'createdAt', key: 'createdAt', width: '35%', render: v => v?.split('T')[0] },
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="mt-4 text-sm text-gray-600">
              {searchReturnNo || searchCreatedAt ? (
                ...
                <p>Showing all {filteredReturns.length} returns</p>
              )}
            </div>
          </div>
        </div>
