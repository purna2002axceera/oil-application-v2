...
const page = () => {
  ...
  // PAGINATED GRN FETCH
  const fetchPaginatedGrns = useCallback(async ({
    page = 1,
    size = 10,
    grnNumber = '',
    invoiceNumber = '',
    ...
    try {
      ...
      if (grnNumber) params.append('grnNumber', grnNumber);
      ...
      const url = `http://localhost:8080/api/grn/search?${params.toString()}`;
      ...
      let filtered = grnArray;
      if (date) {
        const filterDateFormatted = date.format('YYYY-MM-DD');
        filtered = grnArray.filter(grn => {
          const grnDate = grn.createdAt?.split('T')[0] || '';
          return grnDate === filterDateFormatted;
        });
      }

      const grnData = filtered.map(grn => {
  ...
  useEffect(() => {
    getCurrentGrnNumber()
    fetchItems()
    fetchPaginatedGrns({ page: 1, size: pageSize, sortBy, sortDir });
    // eslint-disable-next-line
  }, []);

  // Re-fetch on pagination, sort, or filter change (but NOT on searchText or selectedDate change)
  useEffect(() => {
    fetchPaginatedGrns({
      page: currentPage,
      size: pageSize,
      sortBy,
      sortDir,
      // Do not include searchText or selectedDate here
    });
    // eslint-disable-next-line
  }, [currentPage, pageSize, sortBy, sortDir, items]);
  ...
  const handleCreateGRN = async () => {
    ...
    try {
      await axios.post('http://localhost:8080/api/grn', grnData)
      customToast('success', 'GRN created successfully')
      resetAll()
      fetchPaginatedGrns({ page: currentPage, size: pageSize, sortBy, sortDir })
    } catch (error) {
  ...
  const handleDeleteGrn = async (grnId) => {
    try {
      await axios.delete(`http://localhost:8080/api/grn/${grnId}`)
  ...
  const handleUpdateGRN = async () => {
    try {
      ...
      await axios.put(`http://localhost:8080/api/grn/${editingGrn.id}`, updatedGrnData)
  ...
  const handleSearch = () => {
    setCurrentPage(1);
    let grnNumberParam = '';
    let invoiceNumberParam = '';
    if (searchText.trim()) {
      if (searchText.toUpperCase().startsWith('INV')) {
        invoiceNumberParam = searchText.trim();
      } else if (searchText.toUpperCase().startsWith('PO')) {
        grnNumberParam = searchText.trim();
      } else {
        grnNumberParam = searchText.trim();
        invoiceNumberParam = searchText.trim();
      }
  ...
  // Main table columns
  const mainColumns = [
    { title: 'Purchase Number', dataIndex: 'grnNumber', key: 'grnNumber', width: '25%', sorter: true },
    { title: 'Invoice Number', dataIndex: 'invoiceNumber', key: 'invoiceNumber', width: '20%', sorter: true },
    { title: 'Created At', dataIndex: 'createdAt', key: 'createdAt', width: '20%', sorter: true },
    { title: 'Total Amount', dataIndex: 'totalAmount', key: 'totalAmount', width: '10%', render: (value) => value ? value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : "-", sorter: true },
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="mt-4 text-sm text-gray-600">
            {(searchText || selectedDate) ? (
              <p>
                Showing {filteredGrns.length} of {totalGrns} GRNs
                {searchText && ` matching "${searchText}"`}
                {selectedDate && ` created on ${selectedDate.format('YYYY-MM-DD')}`}
              </p>
            ) : (
              <p>Showing all {totalGrns} GRNs</p>
            )}
          </div>
        </div>
      </div>
